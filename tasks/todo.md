...
>
> **② ✅ 根因已定位（2026-06-14）：負極/GND 回流接觸不良掉 0.5V（ground bounce），非升壓板出力、非正極。修正待處理。**
>
...
>
> **修法（待使用者方便再做）**：只修**負極/地線**（OUT-/B- → ESP32 GND）那條的爛接觸 → 焊死或換 2P 螺絲端子；**正極別碰、電位器別轉**。
> **驗收**：帶載背光全亮時 GND 對 B- < 0.05V（現 0.5V）+ V5IN 對 GND ≈ 4.95V（現 4.5V）+ 連開 5~10 次不閃。
>
