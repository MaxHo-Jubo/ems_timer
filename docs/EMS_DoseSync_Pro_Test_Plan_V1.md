...
| **C** 6 秒通氣節奏（獨立 + OHCA 內恆常顯示） | §5 | ≥ 35 | ≥ 12 | ≥ 13 | §22.5/22.6 |
...
#### 5.1.1 6 秒一口核心定義（V1 §13.3 / §13.5）
- ✅ 核心定義：每 6 秒提示 1 次給氣
- ✅ 第 1 秒 = 給氣提示點（代表此時給一口氣）
- ✅ 第 2~6 秒 = 等待間隔，**不代表給氣、不得在 2~6 秒再次提示**
- ✅ 啟動後秒數從 1 開始，第一個第 1 秒即為第一口給氣提示點
- ✅ 秒數每秒循環：1 → 2 → 3 → 4 → 5 → 6 → 1

#### 5.1.2 1~6 循環提示行為（V1 §13.7 / §13.8）
- ✅ 進入後立即播放秒 1 → 1s 後秒 2 → ... → 秒 6 → 1s 後秒 1（循環）
- ✅ 秒 1：加強提示音 + LED 紅閃 + 畫面反紅
- ✅ 秒 2~6：短提示音、LED 不閃、畫面數字顯示
- ⚠️ **邊界**：循環不停止直到使用者暫停或結束（無自動停止條件）

#### 5.1.3 通氣音量獨立 NVS（V1 §13.9 通氣音量 / §13.10 靜音規則）
- ✅ 通氣音量 NVS key 與系統音量分開
- ✅ 通氣音量 = 0：蜂鳴關閉，**畫面反紅 + LED 紅閃保留 + 秒數視窗仍顯示 1~6**
- ✅ 通氣音量 = 5：最大音量
- ✅ 系統音量 = 3、通氣音量 = 0 → 確認音 / 警報音照常

#### 5.1.4 主按鍵短按暫停 / 繼續（V1 §13.11 / §13.12）
- ✅ 執行中短按主鍵 → 暫停（不需二段確認）
- ✅ 暫停畫面顯示「6秒給氣 / 已暫停 / 主鍵繼續 / 返回結束」
- ✅ 暫停期間：停止秒數循環 / 停止每秒提示音 / 停止第 1 秒加強提示音 / 停止 LED 閃爍 / 不寫入 Timeline
- ✅ 暫停後再短按主鍵 → 恢復節奏，**秒數重新從 1 開始**
- ✅ 恢復後第 1 秒視為新給氣提示點，需觸發畫面反紅 + 加強提示音 + 紅燈閃爍
- ⚠️ **邊界**：暫停時間任意長度，不影響繼續行為

#### 5.1.5 誤觸處理（V1 §13.13）
- ✅ 誤觸主鍵造成暫停可立即再短按主鍵繼續
- ✅ 暫停**不需要二段確認**，以維持即時操作效率
- ✅ 結束功能**必須二段確認**（長按主鍵 3 秒），避免誤觸造成節奏停止

#### 5.1.6 結束與返回鍵（V1 §13.14 / §13.15）
- ✅ 執行中長按主鍵 3 秒 → 結束確認畫面（主鍵確認 / 返回取消）
- ✅ 確認後顯示「通氣節奏 已結束」1 秒 → 回主功能表
- ✅ 執行中按返回鍵 → 提示「如要結束通氣節奏 請長按主鍵」，**不直接結束**
- ✅ 暫停狀態下按返回鍵 → 回到結束確認提示，**不得直接結束節奏**

### 5.2 OHCA 中開啟 6 秒通氣恆常顯示（V1 §14）

> **V4 重大調整**：OHCA 內 6 秒通氣**不再為獨立切換畫面或 overlay 模式**，改為 OHCA 主畫面內的**恆常顯示輔助子計時器區塊**。它**不打斷** OHCA 主流程；EPI 給藥、電擊、Amiodarone、補登完成後，6 秒通氣區塊仍維持顯示。

#### 5.2.1 OHCA 內 6 秒一口核心定義（V1 §14.2 / §14.4）
- ✅ 核心定義與獨立功能一致：每 6 秒提示 1 次給氣
- ✅ 第 1 秒 = 給氣提示點；第 2~6 秒 = 等待間隔
- ✅ 採用**單秒數視窗**顯示，不顯示 1~6 整排數字
- ✅ 開啟後秒數從 1 開始，第一個第 1 秒即為第一口給氣提示點

#### 5.2.2 EPI 到期優先權（V1 §14.8 / pm-dev-spec §7.2）
- ✅ EPI 到期時 OHCA 主畫面顯示「請給藥」+ EPI 高優先警報啟動
- ✅ 6 秒通氣秒數視窗**仍可保留**在畫面下方，但**不得蓋過** EPI 到期警報
- ✅ 6 秒通氣**不得阻止** EPI 給藥確認
- ✅ EPI 警報與第 1 秒給氣提示同時發生：
  - ✅ EPI 警報音優先
  - ✅ 6 秒通氣視覺秒數可繼續顯示
  - ✅ 6 秒通氣提示音可暫時被 EPI 警報覆蓋
  - ✅ 6 秒通氣不得蓋過「請給藥」畫面

#### 5.2.3 OHCA 內 6 秒通氣恆常顯示（V1 §14.3 / §14.7）
- ✅ 開啟後常駐 OHCA 主畫面下方，直到手動關閉或案件結束
- ✅ EPI 給藥確認後，6 秒區塊**仍維持顯示**（不消失、不重置）
- ✅ 電擊 / Amiodarone / 補登完成後，6 秒區塊**仍維持顯示**
- ✅ EPI 倒數重啟後，6 秒區塊**仍維持顯示**
- ✅ EPI ALARMING 時 6 秒區塊**不蓋過**「請給藥」警報
- ✅ 6 秒通氣**不寫入** Timeline，**不列入**案件事件紀錄

#### 5.2.4 開啟、暫停、繼續、關閉（V1 §14.9 三狀態快速功能選單）
- ✅ OHCA 內**不使用主按鍵短按作為 6 秒通氣暫停鍵**，避免與 OHCA 警報消音、選單確認衝突
- ✅ 透過返回鍵 → 快速功能選單操作
- ✅ 三種狀態對應三組選單項目：
  - **未開啟**：開啟 6 秒給氣提示 / 案件簡版總覽 / 返回 OHCA
  - **已開啟且執行中**：暫停 6 秒給氣 / 關閉 6 秒給氣提示 / 案件簡版總覽 / 返回 OHCA
  - **已暫停**：繼續 6 秒給氣 / 關閉 6 秒給氣提示 / 案件簡版總覽 / 返回 OHCA

#### 5.2.5 OHCA 內暫停狀態（V1 §14.10）
- ✅ 暫停後 OHCA 主畫面下方仍顯示 6 秒給氣區塊，狀態改為「已暫停」+ 提示「快速功能可繼續」
- ✅ 暫停期間：停止秒數循環 / 停止每秒提示音 / 停止第 1 秒加強提示音 / 停止 LED 閃爍
- ✅ **不影響** OHCA EPI 倒數
- ✅ **不影響** EPI / 電擊 / Amiodarone / 補登操作
- ✅ **不寫入** Timeline，不列入案件事件紀錄

#### 5.2.6 OHCA 內繼續規則（V1 §14.11）
- ✅ 透過快速功能選擇「繼續 6 秒給氣」→ 恢復節奏
- ✅ 恢復時，**秒數重新從 1 開始**
- ✅ 恢復後第 1 秒視為新給氣提示點，需觸發畫面反紅 + 加強提示音 + 紅燈閃爍

#### 5.2.7 與獨立功能的一致性對照（V1 §14.13）
...
| 第 2~6 秒 | 等待間隔 | 等待間隔 |
| 顯示方式 | 單一目前秒數 | 單秒數視窗 |
| 第 1 秒畫面反紅 / 加強提示音 / 紅燈閃 | 是 | 是 |
| 第 2~6 秒短提示音 / LED 不閃 | 是 / 否 | 是 / 否 |
| 通氣音量 | 0~5 | 0~5 |
| 靜音時視覺提示 | 保留 | 保留 |
| 暫停 / 繼續 | **主鍵短按**（§13.11） | **快速功能選單**（§14.9） |
| 繼續後秒數 | 從 1 開始 | 從 1 開始 |

#### 5.2.8 案件結束規則（V1 §14.12）
- ✅ OHCA 案件結束（LOCKED）後，OHCA 內 6 秒通氣提示**自動停止**
- ✅ 6 秒通氣**不寫入** Timeline
- ✅ 6 秒通氣**不列入**案件事件紀錄
- ✅ 6 秒通氣**不影響** EPI、電擊、Amiodarone 或補登資料
...
#### C.S3 — 獨立模式主鍵短按暫停 / 繼續（V1 §13.11 / §13.12）
- **前置**：主功能表 → 進入「6 秒通氣節奏」並執行至秒 3
- **操作**：(1) 短按主鍵 → (2) 觀察暫停畫面 5s → (3) 再短按主鍵 → (4) 觀察恢復後第一個循環
- **預期**：步驟 1 後立即進入暫停畫面（顯示「6秒給氣 / 已暫停 / 主鍵繼續 / 返回結束」）+ 蜂鳴與 LED 停止；步驟 3 後**秒數從 1 重新開始**，第 1 秒觸發畫面反紅 + 加強提示音 + 紅燈閃爍
- **通過**：暫停期間完全靜音；繼續後秒數從 1 而非從暫停時的 3 接續

#### C.S4 — 獨立模式誤觸快速暫停 / 繼續（V1 §13.13）
- **前置**：主功能表 → 進入「6 秒通氣節奏」並執行至秒 4
- **操作**：(1) 短按主鍵（誤觸暫停）→ (2) 立即（< 0.5s 內）再短按主鍵 → (3) 連續觀察 12s
- **預期**：暫停與繼續過渡流暢無延遲；恢復後秒數從 1 開始；無需任何二段確認
- **通過**：誤觸恢復時間 < 0.5s；後續循環正常

#### C.S5 — 獨立模式長按主鍵 3s 結束二段確認（V1 §13.14 / §13.15）
- **前置**：主功能表 → 進入「6 秒通氣節奏」（執行中或暫停皆可）
- **操作**：(1) 執行中按返回鍵觀察提示 → (2) 長按主鍵 3 秒 → (3) 在結束確認畫面按返回鍵取消 → (4) 再次長按主鍵 3 秒 → (5) 主鍵確認
- **預期**：步驟 1 提示「如要結束通氣節奏 請長按主鍵」（不結束）；步驟 2 進入結束確認；步驟 3 取消回原節奏；步驟 5 顯示「通氣節奏 已結束」1 秒 → 回主功能表
- **通過**：返回鍵不直接結束；結束需主鍵長按 3s + 二段確認

#### C.S6 — 6 秒恆常顯示不蓋警報（V1 §14.7 / §14.8）
- **前置**：OHCA_COUNTDOWN，已開啟 6 秒恆常顯示（從快速功能選單）
- **操作**：(1) 確認主畫面下方有 6 秒節奏單秒數視窗 → (2) 等待倒數歸零進 ALARMING → (3) 觀察畫面布局
- **預期**：步驟 1 主畫面下方常駐「6秒給氣｜已開啟 + 目前秒數」；步驟 3 ALARMING 觸發後「請給藥」覆蓋上半部，6 秒區塊保留下方但不阻礙警報訊息；EPI 警報音優先，6 秒提示音可被覆蓋
- **通過**：「請給藥」清晰可見、6 秒節奏未消失但不蓋過警報

#### C.S7 — OHCA 內三組快速功能選單（V1 §14.9）
- **前置**：OHCA_COUNTDOWN
- **操作**：(1) 短按返回鍵 → 觀察「未開啟」選單 → (2) 選「開啟 6 秒給氣提示」→ (3) 短按返回鍵 → 觀察「已開啟且執行中」選單 → (4) 選「暫停 6 秒給氣」→ (5) 短按返回鍵 → 觀察「已暫停」選單
- **預期**：
  - 步驟 1：選單顯示 3 項（開啟 / 簡版總覽 / 返回 OHCA）
  - 步驟 3：選單顯示 4 項（暫停 / 關閉 / 簡版總覽 / 返回）
  - 步驟 5：選單顯示 4 項（繼續 / 關閉 / 簡版總覽 / 返回）
- **通過**：三組選單依狀態正確切換，項目數量與名稱完全對齊 V4 §14.9

#### C.S8 — OHCA 內主鍵短按不作暫停（反向測試，V1 §14.9）
- **前置**：OHCA_COUNTDOWN，已開啟 6 秒恆常顯示，6 秒區塊執行中
- **操作**：(1) 短按主鍵 → (2) 觀察 6 秒區塊狀態
- **預期**：6 秒區塊狀態維持「已開啟」，秒數循環不中斷；主鍵短按執行 OHCA 主流程的原本功能（如警報消音、選單確認）但**不影響** 6 秒節奏
- **通過**：主鍵短按未觸發 6 秒節奏暫停；秒數循環連續

#### C.S9 — OHCA 內暫停狀態畫面（V1 §14.10）
- **前置**：OHCA_COUNTDOWN，6 秒區塊執行中
- **操作**：(1) 短按返回鍵 → 快速功能選「暫停 6 秒給氣」→ (2) 觀察主畫面下方 6 秒區塊 5s
- **預期**：6 秒區塊狀態改為「6秒給氣｜已暫停 / 快速功能可繼續」；秒數循環 / 提示音 / LED 全部停止；OHCA EPI 倒數秒數**繼續正常遞減**不受影響
- **通過**：暫停顯示正確；EPI 倒數不受影響

#### C.S10 — OHCA 內繼續秒數從 1 開始（V1 §14.11）
- **前置**：C.S9 暫停狀態
- **操作**：(1) 短按返回鍵 → 快速功能選「繼續 6 秒給氣」→ (2) 觀察恢復後第一個循環
- **預期**：6 秒區塊狀態回到「已開啟」+ **秒數從 1 開始**；第 1 秒觸發畫面反紅 + 加強提示音 + 紅燈閃爍
- **通過**：繼續後秒數從 1 而非從暫停時接續

#### C.S11 — OHCA 內事件確認後 6 秒區塊維持顯示（V1 §14.7）
- **前置**：OHCA_COUNTDOWN，6 秒區塊執行中
- **操作**：(1) EPI 二段確認 → 觀察 → (2) 電擊二段確認 → 觀察 → (3) 長按 EPI → Amiodarone → 主鍵確認 → 觀察 → (4) 長按電擊 → 接手前電擊 ×1 → 主鍵確認 → 觀察
- **預期**：四個事件確認後，6 秒區塊**仍維持顯示在主畫面下方**，狀態與秒數循環不重置
- **通過**：四次確認後 6 秒區塊均未消失、未重置秒數

#### C.S12 — EPI 警報與第 1 秒給氣同時發生（V1 §14.8）
- **前置**：OHCA_COUNTDOWN 剩 00:01，6 秒區塊執行中（透過韌體 log 或測試模式對齊第 1 秒到達時間 = ALARMING 觸發時間）
- **操作**：(1) 等待 ALARMING 觸發 → (2) 觀察螢幕與聲音
- **預期**：EPI 警報音優先播放；6 秒通氣視覺秒數可繼續顯示在下方；6 秒提示音可暫時被 EPI 警報覆蓋；「請給藥」畫面未被 6 秒區塊蓋過
- **通過**：EPI 警報優先級正確，6 秒視覺保留但不干擾主警報

#### C.S13 — 案件結束時 6 秒自動停止（V1 §14.12）
- **前置**：OHCA_COUNTDOWN，6 秒區塊執行中
- **操作**：(1) 長按主鍵 3 秒 → 結束前檢查 → 完成並結束案件 → 主鍵確認 → (2) 觀察案件總覽 + Timeline
- **預期**：案件 LOCKED 後 6 秒節奏自動停止；Timeline 不含 6 秒節奏事件；案件總覽不顯示 6 秒節奏統計
- **通過**：6 秒節奏在 LOCKED 後完全停止；事件紀錄純淨
...
| EPI 警報優先 vs 6 秒給氣音覆蓋（V4 §14.8） | 警報音須優先；6 秒提示音可被覆蓋；視覺不蓋過「請給藥」 | 音訊優先級分層；L3 C.S12 對齊 ALARMING 觸發時間驗證 |
...
| 22.5 6秒通氣節奏測試 | C | §5.4（C.S1~S5） |
| 22.6 OHCA 中 6秒通氣恆常顯示 | C | §5.4（C.S6~S13） |
