...
> ⚠️ **本表隨 §9.8 SparkFun Thing Plus 方案（未採用）一起規劃，走的是該板 Qwiic bus（GPIO 8/9），不是現況。** 本專案實際採用 GPIO 42/41——**當前 bus 實況見 `gpio-allocation.md §5.4`「當前 I2C bus 位址表」**。以下補上各裝置的實際狀態（2026-08-22 校準）。

| 裝置 | 位址 | 本專案實際狀態 |
|---|---|---|
| MCP23017 | `0x20` | ⏸ 尚未採購（按鈕擴充候選）|
| 燃料計（本表寫 MAX17048 = SparkFun 板載款）| `0x36` | ✅ 實際採用 **MAX17043** 獨立模組並已上機（2026-08-22 驗收，見 §10.8）|
| ~~SH1106 OLED~~ | ~~`0x3C`~~ | ❌ 已移除——2026-05-08 顯示器改用 TFT ST7789 走 SPI，不再佔 I2C |
| DS3231 RTC | `0x68` | ✅ 已上機（2026-05-24，非本表所寫的「預留」）|
| AT24C32 EEPROM | `0x57` | ✅ 已上機（DS3231 模組附掛；本表漏列，2026-08-22 實機掃描發現）|
