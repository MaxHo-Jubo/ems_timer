...
> ⚙️ 2026-09-06：值域已改為 0=關 / 1=開（見本檔 §15 工程變更表）；靜音規則不變。
...
> ⚙️ **工程變更 2026-09-06（韌體實作現況，上表為原設計，保留不動）**
> 完整根因與影響見 SoT V1 §19.1／§19.3／§19.4／§19.5 的同日註記。
>
> | 設定 | 韌體現況 | 一句話根因 |
> |------|---------|-----------|
> | 螢幕亮度 | **已從選單移除**（NVS 欄位保留） | 背光焊死 3.3V、無可控 GPIO，韌體改不動 |
> | 系統音量 | **0=關 / 1=開**，預設 1；放寬為可靜音 | 主動式蜂鳴器只有響/不響，1~5 級在硬體上完全一樣 |
> | 通氣音量 | **0=關 / 1=開**，預設 1 | 同上；且節奏邏輯本來就只看 `volume > 0` |
>
> 系統音量的開關**只管 UI 操作確認音**；OHCA ALARMING 連續警報、EPI 到期警示、
> 6 秒通氣節奏音都不受它影響（通氣節奏另有自己的開關）。
...
- 獨立模式（從主功能表進入）
- 通氣音量獨立 NVS（原規劃 0~5；2026-09-06 改為 0=關 / 1=開，見 §15 工程變更表）
- OHCA 中快速功能進入（返回鍵）
- EPI 高優先打斷邏輯
- EPI 完成後「返回通氣節奏？」詢問
...
- 螢幕亮度 / 系統音量 / 通氣音量 NVS ✅ 已完成
  （2026-09-06：亮度已移出選單、兩個音量改開/關，見 §15 工程變更表）
- 裝置名稱由 App 寫入 ✅ 已完成
